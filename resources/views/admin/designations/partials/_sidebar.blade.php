                ...
                <div class="col-9 col-xl-9 ">
                    <div class="media text-xl-left">
                        ...
                        <div class="media-body align-self-center text-truncate">
                            <h6 class="text-truncate mb-n1">{{ trans_choice('designations.title', 1) }}</h6>
                            <small class="text-muted">Designations list & search</small>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="tab-content">
                            ...
                            <div id="create-group-details" class="tab-pane fade show active">
                                @if(isset($designation))
                                    <form class="form-horizontal" role="form" method="POST" action="{{ route('designations.update',$designation->id) }}">
                                        {{ method_field('PATCH') }}
                                        @csrf
                                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                                            ...
                                            <input id="first_name" type="text" class="form-control" name="name" value="{{ old('name',$designation->name) }}" />
                                            @if ($errors->has('name'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="status" >
                                                @foreach(trans('statues') as $key=> $status)
                                                    <option value="{{$key}}" @if($key == old('status', $designation->status)) selected @endif>{{$status}}</option>
                                                @endforeach
                                ...
                                @else
                                    <form class="form-horizontal" role="form" method="POST" action="{{ route('designations.store') }}">
                                        {{ csrf_field() }}
